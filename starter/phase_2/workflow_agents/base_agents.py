    ...
    def respond(self, input_text):
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "system", "content": f"""Use the persona defined here : {self.persona}"""},
...
class EvaluationAgent:
    ...
    def evaluate(self, initial_prompt):
        ...
        for i in range(self.max_interactions): # TODO: 2 - Set loop to iterate up to the maximum number of interactions:
            ...
            response = client.chat.completions.create(
                model="gpt-3.5-turbo",
                messages= [{"role": "system", "content": f"You are {self.persona}, a knowledge-based assistant. Forget previous context."},
                {"role": "user", "content": f"Answer based only on this information: {eval_prompt}"}],# TODO: 5 - Define the message structure sent to the LLM for evaluation (use temperature=0)
                temperature=0
            )
            ...
            if evaluation.lower().startswith("yes"):
                ...
                response = client.chat.completions.create(
                    model="gpt-3.5-turbo",
                messages= [{"role": "system", "content": f"You are {self.persona}, a knowledge-based assistant. Forget previous context."},
                {"role": "user", "content": f"Answer based only on this information: {instruction_prompt}"}],# TODO: 5 - Define the message structure sent to the LLM for evaluation (use temperature=0)
...
class RoutingAgent():
    ...
    def route(self, initial_prompt):
        ...
        return best_agent["func"](initial_prompt)
